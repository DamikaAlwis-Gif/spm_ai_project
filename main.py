...
def main():
    try:
        ...
        # Process PDF files and add their content to the vector store
        for file in get_files_list(directory=stored_pdf_files_directory, file_type=".pdf"):
            ...
            if file not in st.session_state.processed_files:
                ...
                save_processed_file(file_name=file, file_type=".pdf")
        ...
        if "conversation_rag_chain" not in st.session_state:
            ...
            retriever_pdf = get_retriever(
                search_type="similarity", vector_store=vector_store_pdf, k=5)
            history_aware_retriver = get_history_aware_retriever(retriever_pdf)
            rag_chain = get_rag_chain(history_aware_retriver)
            
            st.session_state.conversation_rag_chain = conversational_rag_chain(rag_chain)
